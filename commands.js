...
function getNextMilestone(currentXP) {
    const sorted = [...milestones].sort((a, b) => a.xp - b.xp);
    return sorted.find(m => m.xp > currentXP) ?? null;
}
...
async function announceMilestone(data) {
    try {
        ...
        const dedupKey = `${data.discordId}:${data.milestone}`;
        if (recentlyAnnounced.has(dedupKey)) {
            console.log(`[milestone] Duplicate blocked for ${data.discordId} at ${data.milestone} XP`);
            return false;
        }
