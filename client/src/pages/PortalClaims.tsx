...
>
                    ...
                    <td className="px-4 py-2 border border-gray-700">
                      <div className="flex items-center gap-2">
                        <span className="text-[#F19C03] font-semibold">{claim.counter_term.positions_aggregate.aggregate.count.toFixed(1)}</span>
...
<div className="flex gap-2">
                  ...
                  <button onClick={() => handleClaimAction(claim.term_id)} className="bg-[#f19c03] px-4 py-2 rounded-lg text-sm">
                    Oppose
                  </button>
                </div>
  ...
  {toFixed(
    formatEther(
      BigInt(claim.term.total_assets) +
      BigInt(claim.counter_term.total_assets)
    )
  )} TRUST
</span>
  </div>
