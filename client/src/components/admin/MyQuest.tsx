...
export default function MyQuest() {
  ...
  useEffect(() => {
    const selected = localStorage.getItem("selectedQuest");
    if (selected) {
  ...
  if (!quest) return <p className="text-white/60 p-6">No quest selected.</p>;
...
<div className="flex justify-between items-center pr-4 mb-6 border-b border-purple-500/50 pb-3">
  ...
  <div className="flex items-center gap-4">
    ...
    <div className="flex flex-col">
      <h3 className="text-2xl font-bold text-white">{title || "Untitled Quest"}</h3>
