...
export default function DashboardAnalytics() {
  ...
  const [xpAddresses, setXpAddresses] = useState<string>("");
  const [xpSubmitting, setXpSubmitting] = useState(false);
  const [xpResult, setXpResult] = useState<BatchRewardResponse | null>(null);
  ...
  useEffect(() => {
    ...
    apiRequest<LeaderboardResponse>({
      method: "GET",
      endpoint: "/api/admin/leaderboard?page=1&limit=25",
    })
  ...
  const submitSendXp = async () => {
    setXpError(null);
    setXpResult(null);
    const parsedXp = parseInt(xpAmount, 10);
    if (!Number.isFinite(parsedXp) || parsedXp <= 0) {
      setXpError("XP must be a positive number.");
      return;
    }
    const addresses = xpAddresses
      .split(/[\s,;\n]+/)
      .map((a) => a.trim())
      .filter(Boolean);
    if (addresses.length === 0) {
      setXpError("Paste at least one wallet address.");
      return;
    }
    setXpSubmitting(true);
    try {
      const res = await apiRequest<BatchRewardResponse>({
        method: "POST",
        endpoint: "/api/admin/reward-xp-batch",
        data: { addresses, xp: parsedXp },
      });
      setXpResult(res);
    } catch (err) {
      setXpError(err instanceof Error ? err.message : "Failed to send XP.");
    } finally {
  ...
  return (
    <div className="relative z-10 text-white font-geist">
      <div className="max-w-6xl mx-auto relative z-10 space-y-2">
        ...
        <div className="flex items-start justify-between gap-4 mb-4 flex-wrap">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full border border-[#8B3EFE] bg-[#8B3EFE]/20 hover:bg-[#8B3EFE]/40 px-4 py-2 text-xs sm:text-sm font-semibold text-white transition-all shadow-[0_0_20px_rgba(139,62,254,0.3)]"
          >
        ...
        <div className="mt-12 flex flex-col sm:flex-row gap-2 sm:gap-4 w-full">
          {cards.map((card, idx) => (
            <div
              key={idx}
              className="relative overflow-hidden w-full sm:flex-1 rounded-3xl border p-4 flex flex-col justify-between mt-2 sm:mt-4 text-white"
              style={{
                background: "#170F1F",
        ...
        {/* XP in Circulation card */}
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div
            className="rounded-3xl p-5 border relative overflow-hidden text-white"
            style={{
              background: "#170F1F",
              border: "1px solid rgba(131, 58, 253, 0.25)",
              backdropFilter: "blur(8px)",
              ...
              <div>
                ...
                <div className="mt-3 flex items-baseline gap-2">
                  <span className="text-3xl font-bold text-white">
                    {formatNumber(data?.totalXpInCirculation ?? 0)}
                  </span>
                  <span className="text-xs text-white/60">total XP earned</span>
                </div>
              </div>
          ...
          >
            ...
            <div className="relative z-10 flex items-center justify-between">
              <div>
                ...
                <div className="mt-3 flex items-baseline gap-2">
                  <span className="text-3xl font-bold text-white">
                    {formatNumber(data?.totalReferrals ?? 0)}
                  </span>
                  <span className="text-xs text-white/60">referred users</span>
                </div>
              </div>
              <div className="text-5xl">👥</div>
            </div>
        ...
        {/* Leaderboard */}
        <div
          className="mt-10 rounded-3xl p-4 sm:p-6 border relative overflow-hidden text-white"
          style={{
            background: "#170F1F",
            border: "1px solid rgba(131, 58, 253, 0.25)",
            backdropFilter: "blur(8px)",
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg sm:text-2xl font-semibold text-white">Leaderboard</h2>
            <span className="text-xs text-white/60">{leaderboard.length} users</span>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full text-xs sm:text-sm text-left">
              <thead className="text-white/60 uppercase tracking-wider border-b border-white/10">
                <tr>
                  <th className="py-2 pr-2">#</th>
                  <th className="py-2 pr-2">User</th>
                  <th className="py-2 pr-2 whitespace-nowrap">XP</th>
                  <th className="py-2 pr-2 whitespace-nowrap">Nexons Minted</th>
                  <th className="py-2 pr-2 whitespace-nowrap">Lessons Completed</th>
                </tr>
              </thead>
              <tbody>
                {leaderboardLoading ? (
                  ...
                  leaderboard.map((entry) => (
                    ...
                    >
                      <td className="py-3 pr-2 font-bold text-white">{entry.rank}</td>
                      <td className="py-3 pr-2">
                        <div className="flex items-center gap-3 min-w-0">
                          {entry.profilePic ? (
                            ...
                            <div className="w-8 h-8 rounded-full bg-[#8B3EFE]/30 border border-white/20 flex items-center justify-center text-xs font-bold">
                      ...
                      <td className="py-3 pr-2 font-bold text-[#00E1A2]">
                        {formatNumber(entry.xp ?? 0)}
                      </td>
                      <td className="py-3 pr-2 text-white">
                        {formatNumber(entry.noOfMints ?? 0)}
                      </td>
                      <td className="py-3 pr-2 text-white">
                        {formatNumber(entry.lessonsCompleted ?? 0)}
                      </td>
                    </tr>
                  ))
                )}
      ...
      {sendXpOpen ? (
        <div
          ...
          onClick={() => setSendXpOpen(false)}
        >
          <div
            className="w-full max-w-lg rounded-3xl border border-[#8B3EFE]/40 bg-[#170F1F] shadow-[0_0_40px_rgba(139,62,254,0.3)] p-6 relative"
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-2">
              <Send className="w-5 h-5 text-[#8B3EFE]" />
              <h3 className="text-xl font-semibold text-white">Send XP</h3>
            </div>
            <p className="text-sm text-white/60 mb-4">
              Batch-reward XP to user wallets. Max 500 wallets per call.
            </p>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="100"
                  className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-white placeholder:text-white/30 focus:outline-none focus:border-[#8B3EFE] transition-colors"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  value={xpAddresses}
                  onChange={(e) => setXpAddresses(e.target.value)}
                  placeholder="0xabc… one per line, or comma-separated"
                  rows={6}
                  className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-white placeholder:text-white/30 font-mono text-xs focus:outline-none focus:border-[#8B3EFE] transition-colors resize-none"
                />
              </div>
              ...
              {xpResult ? (
                <div className="rounded-xl border border-[#00E1A2]/40 bg-[#00E1A2]/10 px-4 py-3 text-sm text-white space-y-1">
                  <div>
                    Rewarded: <span className="font-bold text-[#00E1A2]">{xpResult.rewarded ?? 0}</span>
                  </div>
                  <div>
                    Not found: <span className="font-bold text-yellow-300">{xpResult.notFound ?? 0}</span>
                  </div>
                  {xpResult.invalid && xpResult.invalid.length > 0 ? (
                    <div>
                      Invalid: <span className="font-bold text-red-300">{xpResult.invalid.length}</span>
                    </div>
                  ) : null}
                </div>
              ) : null}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  onClick={() => setSendXpOpen(false)}
                  className="px-4 py-2 rounded-full border border-white/20 text-white/70 hover:text-white hover:border-white/40 text-sm transition-colors"
                >
                  Close
                </button>
                <button
                  onClick={submitSendXp}
                  disabled={xpSubmitting}
                  className="px-5 py-2 rounded-full bg-[#8B3EFE] hover:bg-[#7b35e6] text-white text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  {xpSubmitting ? "Sending…" : "Send XP"}
                </button>
              </div>
            </div>
          </div>
