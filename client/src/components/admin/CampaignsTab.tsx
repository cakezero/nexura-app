...
export default function CampaignsTab() {
  ...
  return (
    <Card className="p-4 bg-gray-900 text-white w-72 flex flex-col justify-between">
      {/* Title */}
      ...
      {/* Reward Pool (only for active campaigns) */}
      ...
      {/* Details Button */}
      <button
        className="mt-auto px-3 py-1 text-sm bg-purple-600 rounded-lg hover:bg-purple-700 transition"
      ...
      {/* Status Badge */}
      <span className={`px-2 py-1 text-xs rounded mt-2 ${statusColor}`}>
        {status}
      </span>
    </Card>
  );
        ...
        <div className="flex flex-wrap gap-4">
          {activeTab === "all" && (
            <Link
              ...
              className="w-72 p-6 flex flex-col items-center justify-center gap-3
          ...
          {filteredCampaigns.length === 0 ? (
            <p className="text-white/60">No campaigns.</p>
          ) : (
